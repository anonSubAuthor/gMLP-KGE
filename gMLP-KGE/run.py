...
from model.models import gMLP_KGE
...
class Runner(object):
    ...
    def fit(self):
        ...
        scheduler = MultiStepLR(self.optimizer, milestones=[200,300,400], gamma=0.4)
    ...
    def get_model(self):
        model = gMLP_KGE(self.num_ent, self.num_rels, params=self.p)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='lte',
                        help='Set run name for saving/restoring models')
    parser.add_argument('--data', dest='dataset', default='DB100K',
    ...
    parser.add_argument('--t', dest='temperature', type = float ,default=0.03,
                        help='modify loss function')
    #0.01  -0.554
    parser.add_argument('--k', dest='k', type = int ,default=4,
                        help='extend dimension')
    ...
    parser.add_argument('--s_drop', dest='s_drop',
                        default=0.3, type=float, help='Dropout for SpatialGatingUnit')

    parser.add_argument('--g_drop', dest='g_drop',
                        default=0.3, type=float, help='Dropout for gMLPBlock')
    parser.add_argument('--input_drop', dest='input_drop',
                        default=0.3, type=float, help='Dropout after GCN')
    # ConvE specific hyperparameters
    parser.add_argument('--feature_drop', dest='feature_drop', default=0.3, type=float,
                        help='gMLP-KGE: Hidden dropout')
    parser.add_argument('--hidden_drop', dest='hidden_drop', default=0.3, type=float,
                        help='gMLP-KGE: Hidden dropout')

    parser.add_argument('--k_h', dest='k_h', default=400,
